...
try:
    ...
    class ArtNetController:
        ...
        def send_dmx_scatter_gather_batch(
            self,
            world_raster,
            universe_samples,
            channels_per_universe=510,
            ...
            """
            Send ArtNet DMX data using scatter gather mode for multiple universes in a batch.
            Samples from world_raster at the specified coordinates for each universe.

            Args:
                world_raster: The world raster to sample from
                universe_samples: List of (universe, coords) tuples
                channels_per_universe: Number of DMX channels per universe (default 510)
            """
            import numpy as np

            # Process each universe sample
            for universe, coords in universe_samples:
                # Sample pixels from world_raster at the specified coordinates
                data_bytes = bytearray()
                for coord in coords:
                    x, y, z = coord
