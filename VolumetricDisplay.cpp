...
VolumetricDisplay::VolumetricDisplay(int width, int height, int length, int universes_per_layer,
                                     int layer_span, float alpha,
                                     const glm::vec3& initial_rotation_rate,
                                     bool color_correction_enabled,
                                     const std::vector<CubeConfig>& cubes_config, float voxel_scale)
  ...
  for (size_t i = 0; i < cubes_config_.size(); ++i) {
    for (const auto& listener_cfg : cubes_config_[i].listeners) {
      listener_info_.push_back(
          {listener_cfg.ip, listener_cfg.port, static_cast<int>(i), listener_cfg.z_indices});
    }
  }
...
void VolumetricDisplay::setupOpenGL() {
  ...
  GLFWwindow* window = glfwCreateWindow(800, 800, "Volumetric Display", nullptr, nullptr);
  if (!window) {
    glfwTerminate();
