...
struct ListenerThreadInfo {
  ...
  bool is_scatter_gather = false;
  // For scatter gather mode: maps universe -> (channel_sample_index, pixel_offset)
  std::map<int, std::pair<int, int>> universe_to_channel_sample;
  // Reference to channel samples (owned by the config)
  const std::vector<ScatterGatherChannelSample>* channel_samples = nullptr;
};

class VolumetricDisplay {
 public:
  VolumetricDisplay(int width, int height, int length, int universes_per_layer, int layer_span,
                    float alpha, const glm::vec3& initial_rotation_rate,
                    bool color_correction_enabled, const std::vector<CubeConfig>& cubes_config,
                    const std::vector<ScatterGatherCubeConfig>& scatter_gather_cubes_config,
                    float voxel_scale);
  ~VolumetricDisplay();
  void run();
 ...
 private:
  ...
  glm::mat4 computeCubeLocalTransformMatrix(const std::vector<std::string>& world_orientation,
                                            const glm::vec3& size);
  glm::mat4 computeCubeToWorldTransformMatrix(const std::vector<std::string>& world_orientation,
  ...
  int world_width_, world_height_, world_length_;
  size_t num_voxels;
  ...
  bool show_voxel_wireframe = false;
  ...
  std::vector<ScatterGatherCubeConfig> scatter_gather_cubes_config_;
  std::vector<ListenerThreadInfo> listener_info_;
  // Store channel samples for scatter gather (owned here)
  std::vector<std::vector<ScatterGatherChannelSample>> scatter_gather_channel_samples_;
  // Map from coordinate (x,y,z) to voxel index for scatter gather mode
  std::map<glm::ivec3, size_t, Ivec3Comparator> scatter_gather_coord_to_voxel_index_;
